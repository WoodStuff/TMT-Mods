addLayer('l', {
	...
	upgrades: {
		11: {
			...
			effect() {
				eff = player[this.layer].points.plus(2).log(10).add(1);
				if (hasUpgrade('l', 31)) eff = eff.times(upgradeEffect('l', 31));
				eff = softcap(eff, new Decimal(100), 0.25);
				return eff;
			},
		...
		14: {
			...
			effect() {
				eff = player.points.plus(2).log(10);
				return eff;
			},
		...
		24: {
			...
			effect() {
				eff = player.points.plus(2).log(10).max(1.25);
				return eff;
			},
		...
		32: {
			...
			effect() {
				eff = player[this.layer].points.plus(2).log(20).add(1).pow(0.9);
				return eff;
			},
		...
		34: {
			...
			effect() {
				eff = player.l.points.plus(2).log(10);
				return eff;
			},
		...
		53: {
			...
			description: 'Line gain is multiplied by number of line upgrades you have',
