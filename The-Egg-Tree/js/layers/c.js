addLayer('c', {
	...
	effect() {
		lay = hasUpgrade('c', 21) ? player.c.points.add(1.5).pow(0.85).times(1.1125) : player.c.points.add(1).pow(0.65);
		if (hasUpgrade('m', 12)) lay = lay.times(upgradeEffect('m', 12));
		if (hasUpgrade('c', 13)) lay = lay.times(upgradeEffect('c', 13));
		if (hasUpgrade('m', 21)) lay = lay.times(upgradeEffect('m', 21).c);
		return lay;
	},
	effectDescription() {
	...
	upgrades: {
		...
		21: {
			title: 'Layers',
			description: 'Chickens use a better formula',
			cost: new Decimal(15),
			unlocked() {
